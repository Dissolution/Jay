...
public ref struct Charset
{
    ...
    private static char Char(int chunk, int bit)
    {
        return (char)((chunk * 64) + bit);
    }
    private static (int Chunk, int Bit) ChunkBit(char ch) => (ch / 64, ch % 64);

    private readonly Bit64[] _chunks;

    public bool this[char ch]
    {
        get
        {
            var (chunk, bit) = ChunkBit(ch);
            return _chunks[chunk][bit];
        }
        set
        {
            var (chunk, bit) = ChunkBit(ch);
            _chunks[chunk][bit] = value;
        }
    }
    ...
    public int Count
    {
        get
        {
            return _chunks.Sum(ch => ch.PopCount);
        }
    }
    
    public Charset()
    {
        _chunks = new Bit64[ChunkCount];
    }

    public void Add(char ch)
    {
        var (chunk, bit) = ChunkBit(ch);
        _chunks[chunk][bit] = true;
    }
