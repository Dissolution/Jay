...
namespace Jay
{
    ...
    public ref struct Hasher
    {
        ...
        public static Hasher New => new Hasher();
        ...
        {
            return BitOperations.RotateLeft(hash + input * Prime2, 13) * 
                   Prime1;
        }
        ...
        public Hasher Add<T>(T? value)
        {
            AddHash(value?.GetHashCode() ?? 0);
            return this;
        }

        public Hasher Add<T>(T value, IEqualityComparer<T>? comparer)
        {
            if (value is null)
            {
                AddHash(0);
        ...
        public Hasher Add(object? value, IEqualityComparer? comparer)
        {
            if (value is null)
            {
                AddHash(0);
        ...
        public Hasher Add<T>(params T?[]? values)
        {
            if (values is null)
            {
                AddHash(0);
            ...
            else
            {
                ...
                else
                {
                    for (var i = 0; i < len; i++)
                    {
                        AddHash(values[i]?.GetHashCode() ?? 0);
                    }
                }
            }
            return this;
