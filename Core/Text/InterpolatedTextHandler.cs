...
namespace Jay.Text
{
    ...
    public ref struct InterpolatedTextHandler
    {
        ...
        /// <summary>Array rented from the array pool and used to back <see cref="_charSpan"/>.</summary>
        ...
        private void AppendSpanDirect(ReadOnlySpan<char> value)
        {
            if (value.TryCopyTo(_charSpan[_index..]))
            {
                _index += value.Length;
            }
            else
        ...
        private void AppendOrInsertAlignmentIfNeeded(int startingPos, int alignment)
        {
            ...
            int charsWritten = _index - startingPos;
            ...
            if (paddingNeeded > 0)
            {
                ...
                if (leftAlign)
                {
                    _charSpan.Slice(_index, paddingNeeded).Fill(' ');
                }
                else
                {
                    _charSpan.Slice(startingPos, charsWritten).CopyTo(_charSpan[(startingPos + paddingNeeded)..]);
        ...
        /// <summary>Ensures <see cref="_charSpan"/> has the capacity to store <paramref name="additionalChars"/> beyond <see cref="_index"/>.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void EnsureCapacityForAdditionalChars(int additionalChars)
        {
            if (_charSpan.Length - _index < additionalChars)
            {
        ...
        /// <summary>Grows <see cref="_charSpan"/> to have the capacity to store at least <paramref name="additionalChars"/> beyond <see cref="_index"/>.</summary>
        [MethodImpl(MethodImplOptions.NoInlining)] // keep consumers as streamlined as possible
        private void Grow(int additionalChars)
        {
            // This method is called when the remaining space (_charSpan.Length - _index) is
            ...
            Debug.Assert(additionalChars > _charSpan.Length - _index);
            GrowCore((uint)_index + (uint)additionalChars);
        }

        /// <summary>Grows the size of <see cref="_charSpan"/>.</summary>
        [MethodImpl(MethodImplOptions.NoInlining)] // keep consumers as streamlined as possible
        private void Grow()
        {
            // This method is called when the remaining space in _charSpan isn't sufficient to continue
            // the operation.  Thus, we need at least one character beyond _charSpan.Length.  GrowCore
            // will handle growing by more than that if possible.
            GrowCore((uint)_charSpan.Length + 1);
        }

        /// <summary>Grow the size of <see cref="_charSpan"/> to at least the specified <paramref name="requiredMinCapacity"/>.</summary>
        ...
        private void GrowCore(uint requiredMinCapacity)
        {
            ...
            uint newCapacity = Math.Max(requiredMinCapacity, Math.Min((uint)_charSpan.Length * 2, 0x3FFFFFDF));
            ...
            _charSpan[.._index].CopyTo(newArray);
        ...
        public void AppendFormatted<T>(T value)
        {
            ...
            if (value is IFormattable)
            {
                ...
                {
                    int charsWritten;
                    while (!((ISpanFormattable)value).TryFormat(_charSpan[_index..], out charsWritten, default, default)) // constrained call avoiding boxing for value types
                    {
                        Grow();
                    }
                ...
                s = ((IFormattable)value).ToString(default, default); // constrained call avoiding boxing for value types
            }
            else
            {
                s = value?.ToString();
        ...
        public void AppendFormatted<T>(T value, string? format)
        {
            ...
            if (value is IFormattable)
            {
                ...
                {
                    int charsWritten;
                    while (!((ISpanFormattable)value).TryFormat(_charSpan[_index..], out charsWritten, format, default)) // constrained call avoiding boxing for value types
                    {
                        Grow();
                    }
        ...
        public void AppendFormatted(ReadOnlySpan<char> value, int alignment = 0, string? format = null)
        {
            ...
            if (leftAlign)
            {
                value.CopyTo(_charSpan[_index..]);
                _index += value.Length;
                _charSpan.Slice(_index, paddingRequired).Fill(' ');
        ...
        public void AppendFormatted(string? value)
        {
            ...
            if (value is not null &&
                value.TryCopyTo(_charSpan[_index..]))
            {
                _index += value.Length;
            }
            else
        ...
        {
            return UnsuitableException.ThrowEquals(typeof(InterpolatedTextHandler));
        }

        public override int GetHashCode()
        {
            return UnsuitableException.ThrowGetHashCode(typeof(InterpolatedTextHandler));
        }
