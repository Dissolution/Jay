...
namespace Jay.Comparison
{
                                                 ...
                                                 IEqualityComparer<IEnumerable<T>>
    {
        ...
        {
            return Hasher.Create<T>(span);
        }

        public bool Equals(T[]? x, T[]? y) => Equals((ReadOnlySpan<T>)x, (ReadOnlySpan<T>)y);

        public int GetHashCode(T[]? array) => GetHashCode((ReadOnlySpan<T>) array);
