...
namespace Jay.Comparison
{
	...
	public class AlphanumericTextComparer : ITextComparer
	{
		...
		private static int FTBCompare(ReadOnlySpan<char> left, 
		...
		private static int BTFCompare(ReadOnlySpan<char> left, 
		                              ReadOnlySpan<char> right, 
		                              StringComparison comparison)
		{
			int l = left.Length - 1;
		...
		public int Compare(ReadOnlySpan<char> x, ReadOnlySpan<char> y)
		{
			if (_textOrder == TextOrder.FrontToBack)
				return FTBCompare(x, y, _stringComparison);
			return BTFCompare(x, y, _stringComparison);
		}
