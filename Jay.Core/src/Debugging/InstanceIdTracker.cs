...
public static class InstanceIdTracker
{
    public static UIID InstanceId<T>(this T? value)
        where T : class
    {
        return InstanceIdTracker<T>.GetInstanceId(value);
    }
}

public static class InstanceIdTracker<T>
    where T : class
{
    // 0L is for null
